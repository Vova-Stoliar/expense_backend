...
export class AuthController {
    ...
    async signup(@NestCommon.Body() userToSignUp: UserToSignupDto) {
        return this.authService.signup(userToSignUp);
    }

    @Public()
    @NestCommon.Post('login')
    async login(@NestCommon.Body(Pipes.ValidateUserExistence, Pipes.ValidateUserPassword) userToLogin: UserToLoginDto) {
    ...
    async logout(@GetUserPropertyByKey('id') id: BaseUser['id']) {
